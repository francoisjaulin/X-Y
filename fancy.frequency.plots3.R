fancy.freq.plots3 <- function(index,FREQ,my_col="gold") {
  ...
  barplot(P,adj = 0, names.arg = c("0","X","Y","X+Y"),
          main=paste("case=",as.character(index),"\nn tot=",as.character(tot.number),"\nentropy=",as.character(round(entropy,2))),
          cex.names = 1.5,ylim = c(0, UPB),yaxt ='n', ann = FALSE,col = "gray40",font = 2)
  
  segments(0, ADD, 3.8, ADD,col = "red",lwd = 3.0)
  
  axis(3, xaxp = c(3.8, 4.8, 2),labels = c(0,0.5,1),at = seq(3.8,4.8,by = 0.5),cex.axis =
         0.8,font = 2)
  ...
  for (h in (2:(length(BREAKS)))) {
    ...
    col="gray90"
    if (h%%2!=0) col="gray80" 
    rect(3.8, BREAKS[h - 1], 4.8, BREAKS[h],lwd = 1,border = "white",col=col)
  }
  ...
  for (h in (2:(length(BREAKS)))){
    rect(3.8, BREAKS[h - 1], 3.802 + FREQ[h - 1], BREAKS[h],col=my_col,border = "gray40")
  }
